    ...
    run: async (xp, m, {
      ...
      try {
        ...
        const allCmd = Object.values(commands).reduce((a, b) => a + b.length, 0),
              teks = `sapa saya dengan bahasa yang sopan, sedikit teks sambil memperlihat kan menu`,
    ...
    run: async (xp, m, {
      ...
      try {
        ...
        let txt = `${head} ${opb} *P R O F I L E* ${clb}\n`
            ...
            txt += `${body} ${btn} *Pasangan:* ${usr?.cp?.fr?.replace(/@s\.whatsapp\.net$/, '') || 'Tidak punya pasangan'}\n`
            txt += `${foot}${line}\n\n`
            txt += `${readmore}`
