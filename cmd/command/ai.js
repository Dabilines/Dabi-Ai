    ...
    run: async (xp, m, {
      ...
      try {
        ...
        try {
          ...
          imgBuffer = Buffer.from(await finalImg.arrayBuffer())
        } catch (e) {
          const upmedia = await tmpFiles(media),
                fallbackUrl = `https://kaizenapi.my.id/api/ai/live3d?url=${encodeURIComponent(upmedia)}&prompt=${encodeURIComponent(prompt)}&json=true`,
          ...
          const finalImg = await fetch(fallbackJson.result.edited_url)
